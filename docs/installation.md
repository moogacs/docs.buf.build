...
To install just the `buf` binary to `/usr/local/bin` for version `1.1.0`:
...
bash completion, fish completion, and zsh completion to `/usr/local` for version `1.1.0`:
...
GO111MODULE=on GOBIN=/usr/local/bin go install \
  github.com/bufbuild/buf/cmd/buf@v1.1.0
```
